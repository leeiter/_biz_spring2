...
public class AjaxController {
	...
	/*
	 * @ResponseBody
	 * Controller method에서 return하는 값을 변환없이 있는 그대로
	 * 보내라.
	 * 
	 * 일반 method에서는 문자열에 해당하는 view를 찾아서
	 * rendering을 수행하여 client에게 보낸다.
	 */
	@ResponseBody
	...
	/*
	 * 전자정부 관련한 web 서비스에서 사용하던
	 * URL 패턴
	 * 
	 * 일반적으로 서버에 처음 접속을 하면
	 * index.*라는 파일을 보여주고
	 * 메뉴를 클릭하거나 어떤 기능을 선택할 때
	 * 접미사로 .do를 붙여서 연동하도록하는 패턴
	 * *.do 접미사가 붙어있는 URL은 컨트롤러를 통해서
	 * 업무를 처리하고
	 * 그렇지 않은 요청은 모두 무시하게 하는 패턴이였다.
	 * 
	...
	/*
	 * @ResponseBody를 붙이고 객체(클래스)를 return하면
	 * 객체 json으로 변환되어 client에 전송
	 * 단, jackson databind를 pom.xml에
	 ...
	 * 문자열을 return하고 해당하는 문자열.jsp 파일을 views 폴더에서
